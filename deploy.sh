...
show_help() {
    ...
    echo ""
    echo "Commands:"
    echo "  terraform-deploy-s3-misconfig   Deploy misconfigured S3 bucket using Terraform"
    ...
    echo "✅  Use secure configurations for production environments!"
}
...
terraform_deploy_s3_misconfig() {
    ...
    mkdir -p terraform-s3-misconfig-work
    cp terraform-s3-misconfigured.tf terraform-s3-misconfig-work/
    cd terraform-s3-misconfig-work
    terraform init
    terraform plan
    echo ""
...
terraform_deploy_s3_secure() {
    ...
    terraform plan
    echo ""
    echo "✅ This will create a SECURE S3 bucket with public access blocked!"
    read -p "Do you want to continue? (yes/no): " confirm
    if [[ $confirm == "yes" ]]; then
        terraform apply -auto-approve
        echo "✅ Secure S3 bucket deployed with proper security controls!"
    else
        echo "Deployment cancelled."
    fi
    cd ..
...
terraform_destroy_s3() {
    ...
    # Check and destroy misconfigured S3 resources
    if [[ -d "terraform-s3-misconfig-work" ]]; then
        cd terraform-s3-misconfig-work
        terraform destroy -auto-approve
        cd ..
        rm -rf terraform-s3-misconfig-work
        echo "✅ Misconfigured S3 resources destroyed."
    fi
    
    # Check and destroy secure S3 resources
    ...
    # Check legacy terraform-s3-work directory
    ...
    if [[ ! -d "terraform-s3-misconfig-work" && ! -d "terraform-s3-secure-work" && ! -d "terraform-s3-work" ]]; then
