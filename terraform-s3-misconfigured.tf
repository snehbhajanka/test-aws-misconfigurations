...
resource "aws_s3_bucket_public_access_block" "misconfigured_pab" {
  ...
  block_public_policy     = false # Allow public policy for read access
  ignore_public_acls      = true  # Ignore existing public ACLs
  restrict_public_buckets = false # Allow bucket to remain publicly readable
}

# SECURITY FIX: Public read-only ACL (write access blocked)
...
# SECURITY FIX: Public bucket policy allowing read access only (write access blocked)
...
output "security_warnings" {
  value = "WARNING: This bucket has public read access but write access is blocked. Other misconfigurations remain: no encryption, no versioning!"
}
